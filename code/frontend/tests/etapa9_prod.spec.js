...
// Datos de usuarios y credenciales de producción
const CREDENTIALS = {
...
const REPORT_PATH = path.join(ARTIFACT_DIR, 'resultados_etapa9_prod.md');
...
function guardarReporte() {
  let md = `# Reporte de Ejecución - Etapa 9 SGP en Producción\n\n`;
  ...
  md += `\n\n## Casos de Prueba Listos para el Usuario en Producción\n\n`;
  md += `Se crearon y dejaron configurados los siguientes 4 casos listos:\n\n`;
  md += `- **2 Casos de Agenda (Listos para aprobar y generar eventos):**\n`;
  const agendaListos = reportData.resultados.find(r => r.nombre === 'Creación de Casos Listos de Agenda');
  md += `  - ${agendaListos && agendaListos.estado === 'ÉXITO' ? agendaListos.obs : 'No se pudieron crear.'}\n`;
...
test.describe('Suite de Validación Etapa 9 - Producción SGP', () => {
  ...
  test('Validaciones completas de la Etapa 9', async ({ page }) => {
    ...
    // ==========================================
    // CREACIÓN DE CASOS DE PRUEBA EN PRODUCCIÓN
    // ==========================================

    // Auxiliar para crear una solicitud genérica
    ...
    try {
      ...
      resCamposResolucion.obs = `Bloqueo de campos vacíos en calendario validado y aprobación exitosa de la Solicitud ID #${idsCreados.agendaTemp}.`;
    } catch (e) {
    ...
    // ==========================================
    // LIMPIEZA DE CASOS TEMPORALES EN PRODUCCIÓN
    // ==========================================
    try {
