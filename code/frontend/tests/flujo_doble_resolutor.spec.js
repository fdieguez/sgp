...
test.describe('Flujo Avanzado: Doble Resolutor y Carga Completa', () => {
  ...
  test('Ejecutar Flujo Completo: Registro Resolutor 2 -> Asignar a AGENDA -> Carga Completa -> Aprobación Doble', async ({ page }) => {
    ...
    // Cambiar resolutor por defecto
    await page.locator('label:has-text("Defecto") + select').selectOption({ label: `${resolutor2Email} (Resolutor)` });
    ...
    await container2.locator('label:has-text("organizamos") + select').selectOption({ index: 1 });
    await container2.locator('label:has-text("temario") + textarea').fill(`Temario de la actividad del evento: agenda del día, presentaciones, debate y conclusiones ${idUnico}`);
